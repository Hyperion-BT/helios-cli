...
//////////////    Helios   ///////////////
////////////////////////////////////////////////
// Synopsis: Helios is a smart contract DSL for Cardano. 
...
//       helios.deserializePlutusCoreCborHexString(hex: string) -> string
//
//   * Compile Helios program:
//       helios.compileHeliosProgram(programSrc: string, purpose: ScriptPurpose) -> string
//
//   * Compile Helios data:
//       helios.compileHeliosData(programSrc: string, dataExpressionSrc: string) -> string
//
// Disclaimer: I made this available as OpenSource so that the Cardano community can test Helios extensively.
...
exports.unwrapTextEnvelope = exports.serializePlutusCoreProgram = exports.dumpPlutusCoreCborHexString = exports.dumpPlutusCoreCborBytes = exports.deserializePlutusCoreCborHexString = exports.deserializePlutusCoreCborBytes = exports.deserializePlutusCoreBytes = exports.compileHeliosData = exports.compileUntypedHelios = exports.compileHeliosProgram = exports.prettySource = exports.parseHelios = exports.tokenizeUntypedHelios = exports.tokenizeHelios = exports.unsetDebug = exports.setDebug = exports.ScriptPurpose = void 0;
...
const VERSIONS = {
    ...
    "1.0.0": {
        ...
        builtins: [
            ...
            builtinInfo("appendByteString", 0),
...
class Location {
    ...
    syntaxError(msg) {
        throw new HeliosError(this, "SyntaxError", msg);
    }
    typeError(msg) {
        throw new HeliosError(this, "TypeError", msg);
    }
    referenceError(msg) {
        throw new HeliosError(this, "ReferenceError", msg);
    }
    static dummy() {
...
class Tokenizer {
    ...
    readBinaryInteger(ts, start) {
        this.readRadixInteger(ts, start, "0b", c => (c == '0' || c == '1'));
    }
    readOctalInteger(ts, start) {
        this.readRadixInteger(ts, start, "0o", c => (c >= '0' && c <= '7'));
    }
    readHexInteger(ts, start) {
        this.readRadixInteger(ts, start, "0x", c => ((c >= '0' && c <= '9') || (c >= 'a' || c <= 'f')));
    }
...
class HeliosProgram {
    ...
    eval(userScope) {
        ...
        else if (this.purpose_ == exports.ScriptPurpose.Spending) {
        ...
        else if (this.purpose_ == exports.ScriptPurpose.Minting) {
...
class IsStrictlyGt extends BuiltinFunc {
    ...
    static register(registry) {
		...
		func(a, b) {
			${And.generateCode("not(" + And.generateCode("isZero(a)", "isZero(b)") + ")", IsStrictlyGeq.generateCode("isStrictlyGtInnerMaps") + "(a, b)")}
		}`);
    }
}
...
class IsStrictlyLt extends BuiltinFunc {
    ...
    static register(registry) {
		...
		func(a, b) {
			${And.generateCode("not(" + And.generateCode("isZero(a)", "isZero(b)") + ")", IsStrictlyGeq.generateCode("isStrictlyLtInnerMaps") + "(a, b)")}
		}`);
    }
}
...
function buildProgram(ts, purpose) {
    ...
    return new HeliosProgram(decls, purpose);
}
...
const EXPR_BUILDERS = [
    ...
    genBinaryOperatorBuilder('||'),
    genBinaryOperatorBuilder('&&'),
    genBinaryOperatorBuilder(['==', '!=']),
    genBinaryOperatorBuilder(['<', '<=', '>', '>=']),
    genBinaryOperatorBuilder(['+', '-']),
    genBinaryOperatorBuilder(['*', '/', '%']),
    genUnaryOperatorBuilder(['!', '+', '-']),
...
////////////////////////////////////////////////
// Functions for compiling Helios
////////////////////////////////////////////////
function setDebug(d) {
    if (d == undefined) {
        DEBUG = true;
...
function tokenizeHelios(src) {
    let tokenizer = new Tokenizer(src);
    return tokenizer.tokenize();
}
exports.tokenizeHelios = tokenizeHelios;
// same tokenizer can be used for untyped Helios
function tokenizeUntypedHelios(src) {
    let tokenizer = new Tokenizer(src);
    return tokenizer.tokenize();
}
exports.tokenizeUntypedHelios = tokenizeUntypedHelios;
function parseHelios(src, purpose = exports.ScriptPurpose.Spending) {
    let ts = tokenizeHelios(src);
    let program = buildProgram(ts, purpose);
    return program.toString();
}
exports.parseHelios = parseHelios;
// returns string
...
function compileHeliosProgram(typedSrc, purpose = exports.ScriptPurpose.Spending) {
    try {
        let typedTokens = tokenizeHelios(typedSrc);
        ...
        try {
            ...
            let untypedTokens = tokenizeUntypedHelios(untypedSrc);
...
exports.compileHeliosProgram = compileHeliosProgram;
// dumps the Plutus-Core AST
function compileUntypedHelios(untypedSrc) {
    let untypedTokens = tokenizeUntypedHelios(untypedSrc);
    let untypedProgram = buildUntypedProgram(untypedTokens);
    let plutusCoreProgram = new PlutusCoreProgram(DEFAULT_VERSION.map(v => new PlutusCoreInteger(v)), untypedProgram.toPlutusCore());
    return plutusCoreProgram.toString();
}
exports.compileUntypedHelios = compileUntypedHelios;
// output is a string with JSON content
function compileHeliosData(programSrc, dataExprSrc) {
    try {
        let programTokens = tokenizeHelios(programSrc);
        let program = buildProgram(programTokens, exports.ScriptPurpose.Spending); // compiled data is used for Datum and Redeemer, and thus always in Spending context
        let scope = program.linkAndEval();
        // program must make sense before data is compiled
        try {
            let dataExprTokens = tokenizeHelios(dataExprSrc);
...
exports.compileHeliosData = compileHeliosData;
///////////////////////////////////////////////////////
...
exports.deserializePlutusCoreBytes = deserializePlutusCoreBytes;
...
exports.deserializePlutusCoreCborBytes = deserializePlutusCoreCborBytes;
function deserializePlutusCoreCborHexString(hexString) {
    return deserializePlutusCoreCborBytes(hexToBytes(hexString));
}
exports.deserializePlutusCoreCborHexString = deserializePlutusCoreCborHexString;
...
exports.dumpPlutusCoreCborBytes = dumpPlutusCoreCborBytes;
function dumpPlutusCoreCborHexString(hexString) {
    return dumpPlutusCoreCborBytes(hexToBytes(hexString));
}
exports.dumpPlutusCoreCborHexString = dumpPlutusCoreCborHexString;
// returns hex representation of cbor text envelope
function serializePlutusCoreProgram(program) {
...
exports.serializePlutusCoreProgram = serializePlutusCoreProgram;
function unwrapTextEnvelope(hexString) {
    let bytes = hexToBytes(hexString);
    return bytesToHex(unwrapPlutusCoreCbor(unwrapPlutusCoreCbor(bytes)));
}
exports.unwrapTextEnvelope = unwrapTextEnvelope;
